...
def fetch_rafiq_remarks_history(docname):
    history = frappe.db.sql("""
        SELECT
            JSON_UNQUOTE(JSON_EXTRACT(changes.value, '$[1]')) AS old_value,
    ...
    remarks_history = ""
    for row in history:
        remarks_history += f"Changed by {row.changed_by} on {row.changed_on.strftime('%Y-%m-%d %H:%M:%S')}\n"
        remarks_history += f"Old: {row.old_value or 'None'}\nNew: {row.new_value or 'None'}\n---\n"
